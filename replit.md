...
> **Никакого Shell.** Только UI — три шага, потом Run.

### 1. Добавь PostgreSQL базу данных
**Tools → Database** → создай PostgreSQL БД.
...
### 3. Нажми Run
Сервер сам:
- применит схему БД (`drizzle-kit push`)
- соберёт код
- зарегистрирует Telegram webhook

### 4. Активируй бота
Напиши `/start` боту в Telegram. Всё готово.

> **Shell не нужен никогда.** Схема БД применяется автоматически при каждом старте.
