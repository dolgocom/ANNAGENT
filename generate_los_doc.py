...
run = meta.add_run(f"Версия: 1.1  |  Дата: {datetime.date.today().strftime('%d.%m.%Y')}  |  Конфиденциально")
run.font.size = Pt(11)
...
bullets = [
    ...
    "контролирует отношения, здоровье и коммуникации в одном месте;",
...
add_paragraph(
    ...
    "Система состоит из 6 специализированных ИИ-агентов под управлением единого Master Orchestrator. "
...
add_heading("2.1 Концепция: 6 агентов + 1 Orchestrator", 2)
add_paragraph(
    "Система построена по принципу «звезда»: в центре — Master Orchestrator (центральный агент-координатор), "
    "вокруг него — 6 специализированных агентов, каждый из которых является экспертом своего домена."
)
...
add_paragraph("Типология встреч по когнитивной нагрузке:", bold=True)
loads = [
...
add_paragraph("Логика анализа:", bold=True)
logic = [
    "Пороговые алерты (срочные): Readiness Score < N → флаг на перегруз, предложение переноса тяжёлых встреч",
    "Паттерны (проактивные): Анализ динамики за 7 дней; предупреждение до наступления перегруза",
    "Приоритет субъективного ввода: при конфликте данных Oura vs. субъективный ввод — победа за субъективным (флаг data_conflict)",
]
for l in logic:
    doc.add_paragraph(l, style='List Bullet')
...
add_paragraph(
    "Основная цель: центральный мозг системы — агрегирует контекст всех агентов, проводит "
    "мультилинзовый анализ решений, приоритизирует задачи дня, выступает арбитром при конфликтах."
)
...
lenses = [
    ...
    ("Коммуникационная", "Communication Intel: контекст переговоров и профайлинг"),
]
...
add_paragraph("Триггеры вызова:", bold=True)
triggers = [
    "07:00 МСК (ежедневно): автоматический утренний брифинг",
    "22:00 МСК (ежедневно): автоматический вечерний дайджест",
...
p.add_run(
    ...
    "для стратегических решений. Приоритеты: 1) Решение по сделке с Петровым (дедлайн 2 дня); "
...
for sys, desc in systems:
    ...
    p.add_run(desc)
add_paragraph("Входные данные: дата, время и место рождения; имя (для нумерологии). Вносятся при настройке бота.")
add_paragraph("Триггер: каждое утро автоматически — до начала рабочего дня.")
...
funcs_health = [
    "Динамика показателей: выявляет устойчивые отклонения в анализах крови и тестах",
    "Ресёрч препаратов: при новом назначении — состав, механизм действия, побочные эффекты, взаимодействия",
    "Напоминания о визитах: контролирует плановые обследования и интервалы между ними",
    "Мониторинг приёма: фиксирует факт приёма медикаментов / добавок",
]
for f in funcs_health:
    doc.add_paragraph(f, style='List Bullet')
add_paragraph("Ограничения: не ставит диагнозов; не назначает медикаменты; только трекает и исследует назначенное врачом.")
...
funcs_net = [
    ...
    "Динамика отношений: фиксирует статус (развиваются / стагнируют / охлаждаются); флаг при долгом отсутствии касания",
    "Напоминания: дни рождения (за 7, 2 дня и в день), религиозные и государственные праздники, годовщины",
]
for f in funcs_net:
    doc.add_paragraph(f, style='List Bullet')

# 3.6 Communication Intel
add_heading("3.6 Communication Intelligence Agent  [⏳ Фаза 2]", 2)
add_paragraph(
...
add_paragraph("Источники данных:", bold=True)
add_paragraph("• PLAUDE: аудиофайл + текстовое саммари встречи — загружает Аня вручную")
add_paragraph("• Network Agent: профили участников (если контакт есть в базе)")
add_paragraph("• Ручной ввод Ани: контекст встречи — участники, цель, дата")
...
modules = [
    ...
    "Скрытые конфликты: противоречия между словами, избегаемые темы, напряжение в диалоге",
    "Верификация искренности: несоответствия в логике аргументов (фиксируются как признаки, не доказательства)",
    "Профайлинг личности: тип личности, мотивации, вероятные слабые точки собеседника",
    "Юридический анализ: риски позиции, уязвимости, рекомендации по следующим шагам",
]
for m in modules:
    doc.add_paragraph(m, style='List Bullet')

add_paragraph("Политика хранения:", bold=True)
add_paragraph("Хранятся: результаты анализа (структурированно), краткие сведения об участниках, дата и факт анализа.")
add_paragraph("НЕ хранятся: аудиозаписи, полные транскрипции, чувствительные детали переговоров.")

add_paragraph("Пример вывода:", bold=True)
p = doc.add_paragraph()
p.add_run(
    "«Встреча: переговоры по сделке, 15.01.2025. Участник: Иван Петров. "
    "Манипуляция: зафиксирована техника искусственного дефицита времени ("нам нужно решение сегодня"). "
    "Скрытый конфликт: устойчивое избегание прямого ответа по финансовым гарантиям (3 обхода за разговор). "
    "Рекомендация: не принимать решение под давлением; запросить письменные гарантии.»"
).italic = True
...
layers = [
    ("⚡ Рабочая (Working)", "Текущий разговор, активный контекст", "LLM context window", "Пока идёт разговор"),
    ("📖 Эпизодическая (Episodic)", "Что происходило: «в июле летел в Бали»", "PostgreSQL + векторные эмбеддинги", "Навсегда"),
...
add_paragraph(
    ...
    "и сохраняет их с возможностью семантического поиска. Лицензия: Apache 2.0. "
    "Работает поверх PostgreSQL с расширением pgvector — инфраструктура уже есть в проекте."
)
...
stores = [
    ...
    ("Graph Store", "Хранит связи: «Пётр — деловой партнёр», «Ahmed — ключевой клиент, Дубай»"),
...
add_paragraph("\nКак это работает автоматически:", bold=True)
add_paragraph(
    "Аня говорит: «Он предпочитает переговоры до 12:00» → "
    "Mem0 извлекает факт → сохраняет в Vector Store + KV Store → "
    "при следующем запросе оркестратор получает этот контекст → персонализированный ответ."
)
...
for cell in hdr5:
    for para in cell.paragraphs:
        for run in para.runs:
            run.bold = True
comparisons = [
    ("✅ Mem0 (выбран)", "Open-source, pgvector, автоизвлечение фактов, Python-библиотека, идеален для личных ассистентов", "Граф менее мощный, чем у Zep"),
    ("⚠️ Zep / Graphiti", "Мощный темпоральный граф связей", "Требует Neo4j, сложнее в настройке, корпоративное решение"),
...
add_paragraph(
    ...
    "система работает на данных Oura без субъективного ввода."
)

add_heading("5.2 День — Реактивный режим", 2)
reactive_events = [
    "Новая встреча в календаре → Neuro & Bio оценивает нагрузку; при необходимости — рекомендация по перестановке",
    "Аня загружает запись из PLAUDE → Communication Intelligence проводит анализ переговоров",
    "Аня описывает дилемму или решение → Decision Support собирает контекст и выдаёт мультилинзовый анализ",
    "Тревожный тренд в Oura → Health Agent проверяет медицинский контекст и флагирует",
    "Аня вводит данные вручную (анализы, препараты, информацию о контакте) → агент обновляет базу",
]
...
doc.add_paragraph()
add_heading("6.1 Необходимые API-ключи для запуска", 2)
...
for cell in hdr9:
    for para in cell.paragraphs:
        for run in para.runs:
            run.bold = True
apis = [
    ("Telegram Bot Token", "Через @BotFather в Telegram → команда /newbot"),
    ("OpenAI API Key", "platform.openai.com → раздел API Keys"),
...
add_paragraph("Состав: Neuro & Bio Agent + Decision Support Agent + Telegram-бот + Oura Ring.")
phase1 = [
    ...
    "Алерты при низком Readiness Score + рекомендации по расписанию",
    "Утренний брифинг 07:00 МСК (автоматически)",
    "Вечерний дайджест 22:00 МСК (автоматически)",
...
p.add_run("Работающий MVP — получает персонализированный утренний брифинг и рекомендации по расписанию на основе физического состояния.")

doc.add_paragraph()
add_heading("Фаза 2 — Полная интеграция  ⏳", 2)
add_paragraph("Состав: все 6 агентов + полная интеграция источников данных.")
phase2 = [
    "Esoteric Intelligence Agent (ввод натальных данных через бот)",
...
security_rules = [
    ...
    "Эзотерические рекомендации не заменяют решения Ани/босса; всегда сопровождаются обоснованием",
]
...
glossary = [
    ...
    ("Mem0", "Open-source фреймворк памяти для ИИ-агентов (Apache 2.0, pgvector)"),
    ...
    ("high_risk", "Флаг Communication Intelligence Agent при высоком риске в переговорах"),
    ("data_conflict", "Флаг при расхождении данных Oura и субъективного ввода Ани"),
]
...
run = ver.add_run(f"LOS v1.1  |  {datetime.date.today().strftime('%d.%m.%Y')}")
run.font.color.rgb = RGBColor(150, 150, 150)
run.font.size = Pt(10)
